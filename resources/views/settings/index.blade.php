...
<div class="container-fluid">
            ...
            <form method="POST" action="{{ route('settings.update') }}">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="time" name="working_hours_start" value="{{ $settings['working_hours_start'] }}" class="form-control" style="border-radius: 12px;" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-semibold">Giờ kết thúc</label>
                        <input type="time" name="working_hours_end" value="{{ $settings['working_hours_end'] }}" class="form-control" style="border-radius: 12px;" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-semibold">Ngưỡng đi muộn (phút)</label>
                        <input type="number" name="late_threshold" value="{{ $settings['late_threshold'] }}" class="form-control" style="border-radius: 12px;" required>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-semibold">Ngưỡng về sớm (phút)</label>
                        <input type="number" name="early_leave_threshold" value="{{ $settings['early_leave_threshold'] }}" class="form-control" style="border-radius: 12px;" required>
                    </div>
                </div>
                <div class="mt-4">
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="py-3 px-4 fw-semibold">{{ $holiday->name }}</td>
                            <td class="py-3 px-4">
                                @if($holiday->end_date && $holiday->end_date != $holiday->date)
                                    <div>{{ $holiday->date->format('d/m/Y') }}</div>
                                    <div class="text-muted small">đến {{ $holiday->end_date->format('d/m/Y') }}</div>
                                    <div class="badge bg-info mt-1">
                                        {{ \Carbon\Carbon::parse($holiday->date)->diffInDays($holiday->end_date) + 1 }} ngày
                                    </div>
                                @else
                            ...
                            <td class="py-3 px-4">{{ $holiday->compensate_to ? $holiday->compensate_to->format('d/m/Y') : '-' }}</td>
                            <td class="py-3 px-4 text-end">
                                <button type="button" class="btn btn-sm btn-outline-primary me-2" onclick="editHoliday({{ $holiday->id }}, '{{ $holiday->name }}', '{{ $holiday->date->format('Y-m-d') }}', '{{ $holiday->end_date ? $holiday->end_date->format('Y-m-d') : '' }}', {{ $holiday->is_recurring ? 'true' : 'false' }}, '{{ $holiday->compensate_to ? $holiday->compensate_to->format('Y-m-d') : '' }}')">
                                    <i class="bi bi-pencil"></i>
                                </button>
                                <form action="{{ route('settings.holidays.destroy', $holiday) }}" method="POST" class="d-inline" onsubmit="return confirm('Bạn có chắc chắn muốn xóa ngày lễ này?');">
                                    @csrf
                                    @method('DELETE')
    ...
    <script>
        ...
        function openHolidayModal() {
            ...
            document.getElementById('holidayForm').action = '{{ route("settings.holidays.store") }}';
        ...
        function editHoliday(id, name, date, endDate, isRecurring, compensateTo) {
            ...
            document.getElementById('holidayForm').action = '{{ route("settings.holidays.update", ":id") }}'.replace(':id', id);
